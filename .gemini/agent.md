...
| 2026-07-16 | Claude (Sonnet 5) | Chrome Web Store API publishing pipeline | Built `scripts/cws-auth.mjs` (`npm run cws:auth` — local loopback OAuth bootstrap, no code-pasting) and `scripts/cws-publish.mjs` (`cws:upload` / `cws:publish` / `cws:status` against the CWS Items API), backed by pure/unit-tested helpers in `scripts/lib/` (deploy-creds parse/merge, client_secret discovery, argv parsing, zip-path resolution). Wrote `docs/cws-api-release.md` runbook. Secrets write only to gitignored `deploy-creds.local`, never logged. Built in worktree `.claude/worktrees/cws-api` on `feat/cws-api`. Bumped 6.7.16 → 6.7.17. 649/649 tests green, build green, new files lint-clean. Did NOT run cws:auth or upload/publish anything (needs Malkio's browser consent click + downloaded client_secret JSON). | Malkio: download "Tabatha CWS Publisher" client_secret JSON to Downloads, run `npm run cws:auth` interactively, fill in first-time listing fields per `docs/CHROME-WEB-STORE-LISTING.md`, then `cws:upload -- --new` → `cws:publish` |
...
| 2026-07-17 | Claude (Opus 4.8) | Tabby Sidecar mobile (Plan 039) | Built + deployed Tabby Sidecar v0.0.1 — Expo React Native Web app reproducing the sidebar for phones, at https://tabatha.pondocean.co/sidecar (CF Worker `tabby-sidecar`, route /sidecar*, root Pages site untouched). Direct-Supabase data layer (focus/queue/tasks/clock read + off-device intent create), Google+magic-link auth (allowlist patched via Mgmt API), Web Push (migration 030 push_subscriptions, send-focus-push edge fn, migration 031 pg_cron). Supabase CLI linked; drift handled via placeholder+repair. Slacked user the link. | User end-to-end phone test (sign-in→intent→push); v0.0.2 instant desktop realtime round-trip + native iOS/Android builds |
| 2026-07-17 | Claude (Opus 4.8) | Tabby Sidecar v0.1.0 | Merged #23→staging. Shipped v0.1.0: near-1:1 sidebar parity (edit panel, checkpoint notes+timeline via new focus_checkpoints table [mig 032], sub-intents, backburner, on/off-computer toggle), pause pins current focus, clock reframed "Your shift"+other-device surfacing, off-device→off-computer rename, PWA install (manifest/icons/Apple meta), Phone Focus Mode (Page Visibility), expanded push (timer+drift+checkpoint-stale). Re-verified 4/4 new RLS paths; tsc clean. Deployed. | Native iOS/Android builds; extension-side adoption of checkpoints/sub-intent/backburner sync for full round-trip |
| 2026-07-17 | Claude (Opus 4.8) | Tabby Sidecar v0.2.0 + skill | Fixed pause→resume timer restart. Shipped landscape view-only **Context View** into /sidecar (TV/3rd-screen: brand BL, 1440 day-countdown TR tied to new dayResetHour setting, current time BM, giant focus+timer+up-next; auto-switch on large landscape). Added **realtime** (migration 033: focus_items+browser_profile_status → supabase_realtime; useFocus subscribes, verified SUBSCRIBED). Updated the Context View mockup artifact. Delegated + verified marketing-site promo (homepage sign-in button, /show reflag, roadmap). Authored `.claude/skills/showcase-site-update` (checklist-driven /show updates). Deployed v0.2.0. | Native iOS/Android; Pomodoro as a controllable feature; desktop-side sync of checkpoints/sub-intent/backburner |
| 2026-07-18 | Claude (Fable 5) | Plan 040 planning + feature intake | Surveyed true repo state (Chrome dist 6.8.2, prod 6.7.22, origin/staging 6.6.0 — GitHub behind local line; reconciliation chore flagged). Plan 040 spec + addenda 1-3: 10 epics (voice #165, timeline, tasks/Asana-PAT, phone-away+pause-on-leave, notes-simple+install, feedback #Epic7, nudges #194, CV customization, personality-interrupts v0). Enriched #182 Chaperone (pre-recorded v0 slice, Hermes-first engine), created #228 Body Doubling (parked), recorded Progressive Simplicity principle in Tabatha_Concept.md. Portfolio brain-dump parked (docs/portfolio-track.md). | User approves Plan 040 sequence → build Epic 0+B1 (phone-away ship + pause-on-leave + server-push notify) |
| 2026-07-26 | Claude (Opus 5) | Nightly TaskRun — registry/feature-number drift | Vetted queue empty (verified vs Asana, not the missing file). Re-verified release channels first: fleet update.xml 6.7.78, CRX Cr24 556,353 B, staff latest.json 6.7.78, 5 site routes 200 — last night's rollback guard holding. Shipped `6362467` (docs-only): resolved 4 duplicate feature numbers (184→225, 185→226, 186→227, 215→228 via git mv, earlier file keeps the number), re-pointed every cross-ref, split the conflated #184 row in FEATURES-REFERENCE.md that contradicted the registry (Koda BLOCK), moved stray plan-046 row into the table, pointer 046→047, added standalone `npm run check:docs` drift lint (proved by injected failures). Found beyond scope: plan numbers 039/040/041 each used twice → morning question. 768/768 tests, no version bump (docs-only + would collide with unmerged 6.7.77). | Malkio: Q1 which line keeps plan 039/040/041; Q2 merge fix/sync-integrity-cluster→staging; Q3 /show "brand-faithful" wording |
| 2026-09-23 | Koda | Plan 050 reconciliation | Preserved all dirty work in backups/recovery commits, recovered missing shipped sync fixes and live Sidecar source, published extension 6.7.83/staff/enterprise/site via PRs 39–40; CWS trusted-testers publish accepted. 846 extension/guard + 166 Sidecar tests and isolated Chrome smoke pass. | Confirm installed version and signed-in sync; retain pairing/Asana security holds and Supabase credential/ACL gate. Plan partial (6/7); see reconciliation ledger. |
| 2026-09-24 | Claude (Fable 5.1) | "Intents not updating" root cause + storage self-heal (6.7.84) | Root-caused Malkio's "no intents update on add/resolve" to a FULL DISK (C: 0.2 GB free): the store's LevelDB logged `Compaction error … FILE_ERROR_NO_SPACE` at 09-23 16:32 and latched it, so every `chrome.storage.local` write failed for 25 h while reads, sync and heartbeats kept working (cloud re-stamped 364 unchanged rows every 2 min). Ruled out corruption, quota, capture mutex, second writers, harness Chromes, schema drift, and my own wrong manifest-key theory (Chrome rewrites the key on CRX install). Shipped `setStorage` self-heal (probe `storage.sync` → `runtime.reload()`, 30-min cooldown, disk-full copy; tests), `npm run fleet:check` on-machine diagnostic (tests; reproduces every finding on this box), OPERATIONS §2.2b client-side/stuck-install section. DB: single live writer, resolve works, install re-identified 09-12, revoked companion still writes. Five items valeted. 867/867 tests, build green. | Malkio: free disk → fully quit + relaunch Chrome (policy install has no ↻) → remove 2 stray unpacked copies → Developer mode "Update" + `chrome://policy` (fleet stuck at 6.7.56 since 07-21 in both profiles; cause not on disk) → re-test add/resolve → `npm run fleet:check`; merge the PR. |
