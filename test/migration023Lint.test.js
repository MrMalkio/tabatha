// NB-01/NB-02 — migration-lint guard for 023_schedule_profiles_requirements.sql
...
const migrationPath = fileURLToPath(
  new URL('../supabase/migrations/023_schedule_profiles_requirements.sql', import.meta.url)
);
...
test('023 creates the four NB-01/NB-02 tables idempotently', () => {
...
test('023 enables RLS on every new table', () => {
  for (const t of TABLES) {
...
test('023 adds work_profile_type to org_members (membership grain, per Koda)', () => {
...
test('023 cadence CHECK is exactly daily/weekly/monthly (both tables)', () => {
...
test('023 request kind CHECK is slot_change/shift_hours/make_up', () => {
  assert.deepEqual(
...
test('023 request status CHECK is pending/approved/rejected', () => {
  assert.deepEqual(
...
test('023 shortfall resolution CHECK is unresolved/made_up/shifted/excused', () => {
  assert.deepEqual(
...
test('023 work_profile_type CHECK is dedicated_hours/self_managed', () => {
  assert.deepEqual(
...
test('023 weekday convention: 0..6 SMALLINT on schedule slots', () => {
  assert.match(sql, /weekday SMALLINT NOT NULL CHECK \(weekday BETWEEN 0 AND 6\)/,
    'weekday must be SMALLINT constrained to 0..6');
});

test('023 shortfall ledger has the idempotency unique index (per prompt-time inserts)', () => {
  assert.match(sql,
...
// ── RLS consumes the 022 helpers ────────────────────────────

test('023 policies use migration 022 helpers (current_profile_id / can_manage_profile)', () => {
...
test('023 members can INSERT their own change requests but only as pending', () => {
...
test('023 status decisions (UPDATE) are manager-scope only', () => {
...
// ── RPC hardening (020/022 precedent) ───────────────────────
...
test('023 defines exactly the four NB-01/NB-02 RPCs', () => {
...
for (const [name, argTypes] of Object.entries(FUNCTIONS)) {
  test(`023 hardening — tabatha.${name}`, () => {
...
test('023 function bodies are schema-qualified (safe under empty search_path)', () => {
