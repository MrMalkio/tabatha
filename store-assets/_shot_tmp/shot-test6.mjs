...
const { targetId } = await browser.send('Target.createTarget', { url: 'chrome-extension://admccjkmockfdflocgggjfgdacdodkdf/home.html' });
const { sessionId } = await browser.send('Target.attachToTarget', { targetId, flatten: true });
await browser.send('Page.enable', {}, sessionId);
await sleep(2000);
const shot = await browser.send('Page.captureScreenshot', { format: 'png' }, sessionId);
writeFileSync('test6.png', Buffer.from(shot.data, 'base64'));
