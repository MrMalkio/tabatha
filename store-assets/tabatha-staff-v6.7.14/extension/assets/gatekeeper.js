(async()=>{let e;try{e=await chrome.runtime.sendMessage({type:`CHECK_CONTEXT_NEEDED`})}catch{return}if(!e||!e.needed)return;let t=e.inheritedContext||``;e.inheritedIntent;let n=e.contextSource||null,r=[],i=[],a=[],o=3,s=!0,c=10;try{let e=await chrome.runtime.sendMessage({type:`GET_FOCUS_ENGINE`});e?.focusEngine?.items&&(r=Object.values(e.focusEngine.items).filter(e=>e.focusState===`active`||e.focusState===`paused`).sort((e,t)=>new Date(t.createdAt)-new Date(e.createdAt)))}catch{}try{let e=await chrome.storage.local.get([`intentHistory`,`intentPresets`,`settings`]);if(o=e.settings?.inheritItemCount||3,s=e.settings?.inpopStrictMode!==!1,c=e.settings?.inpopBlurStrength??10,e.intentHistory){let t=new Date().toDateString(),n=new Set,a=new Set(r.map(e=>e.label.toLowerCase()));for(let r of e.intentHistory){let e=r.context??r.newContext;if(e&&new Date(r.timestamp).toDateString()===t&&!n.has(e.toLowerCase())&&!a.has(e.toLowerCase())&&(n.add(e.toLowerCase()),i.push(e),i.length>=5))break}}if(e.intentPresets?.persistent){let t=new Set(r.map(e=>e.label.toLowerCase())),n=new Set(i.map(e=>e.toLowerCase()));a=e.intentPresets.persistent.filter(e=>!t.has(e.label.toLowerCase())&&!n.has(e.label.toLowerCase())).map(e=>e.label)}}catch{}if(r=r.slice(0,o),await new Promise(e=>{if(document.body)return e();let t=new MutationObserver(()=>{document.body&&(t.disconnect(),e())});t.observe(document.documentElement,{childList:!0}),setTimeout(()=>{t.disconnect(),e()},3e3)}),!document.body)return;let l=document.createElement(`div`);l.id=`tabatha-gatekeeper-host`,Object.assign(l.style,{position:`fixed`,top:`0`,left:`0`,width:`100vw`,height:`100vh`,zIndex:`2147483647`,backgroundColor:`rgba(0,0,0,${s?.85:.6})`,backdropFilter:`blur(${c}px)`,pointerEvents:`auto`});let u=l.attachShadow({mode:`closed`});document.documentElement.appendChild(l),s&&(document.body.style.overflow=`hidden`);let d=document.createElement(`style`);d.textContent=`
  ...
  `,u.appendChild(f);let v=u.getElementById(`context`),y=`human`;u.querySelectorAll(`.who-opt`).forEach(e=>{e.onclick=()=>{y=e.getAttribute(`data-who`),u.querySelectorAll(`.who-opt`).forEach(t=>t.classList.toggle(`selected`,t===e)),v&&v.focus()}});let b=async()=>{if(y===`agent`)try{let e=await chrome.runtime.sendMessage({type:`GET_CURRENT_TAB_ID`});await chrome.runtime.sendMessage({type:`START_AGENT_SESSION`,scope:`tab`,tabId:e?.tabId??null,agentName:`manual`,source:`manual`})}catch{}},x=()=>{l.remove(),document.body&&(document.body.style.overflow=``)},S=(e,t={})=>chrome.runtime.sendMessage({type:`LOG_INTENT_ACTION`,action:e,url:window.location.href,domain:location.hostname,...t}).catch(()=>{}),C=async(e,t=null)=>{let n=v.value.trim(),r=n||e;await chrome.runtime.sendMessage({type:`SET_TAB_CONTEXT`,context:r,category:`work`,intent:t?`inherited_from_focus`:`preset`}).catch(()=>{}),t?await chrome.runtime.sendMessage({type:`ASSOCIATE_TAB_WITH_FOCUS`,focusId:t}).catch(()=>{}):n&&n.toLowerCase()!==e.toLowerCase()&&await chrome.runtime.sendMessage({type:`SET_TAB_CONTEXT`,context:n,category:`work`,intent:`child_of_preset`,parentContext:e}).catch(()=>{}),await b(),await S(t?`inherit`:`continue`,{context:r,focusId:t,parentContext:n?e:null}),x()};u.getElementById(`continue`).onclick=async()=>{let e=v.value.trim();if(!e){v.style.borderColor=`#ff6b6b`,v.placeholder=`Please describe your intent...`;return}await chrome.runtime.sendMessage({type:`SET_TAB_CONTEXT`,context:e,category:`work`,intent:`user_defined`}).catch(()=>{}),await b(),await S(`continue`,{context:e}),x()},u.getElementById(`side-quest`).onclick=async()=>{let e=v.value.trim()||`Side Quest`;await chrome.runtime.sendMessage({type:`START_SIDE_QUEST`,context:e,minutes:5}).catch(()=>{}),await S(`side_quest`,{context:e}),x()},u.getElementById(`sugar-box`).onclick=async()=>{await chrome.runtime.sendMessage({type:`ADD_TO_SUGAR_BOX`,url:window.location.href,title:document.title}).catch(()=>{}),await S(`sugar_box`),x();try{let e=await chrome.runtime.sendMessage({type:`GET_CURRENT_TAB_ID`});e?.tabId&&await chrome.runtime.sendMessage({type:`CLOSE_TAB`,tabId:e.tabId})}catch{window.close()}},u.getElementById(`park`).onclick=async()=>{await chrome.runtime.sendMessage({type:`PARK_TAB`,url:window.location.href,title:document.title}).catch(()=>{}),await S(`park`),x();try{let e=await chrome.runtime.sendMessage({type:`GET_CURRENT_TAB_ID`});e?.tabId&&await chrome.runtime.sendMessage({type:`CLOSE_TAB`,tabId:e.tabId})}catch{window.close()}},u.getElementById(`later`).onclick=async()=>{let e=v.value.trim()||document.title;await chrome.runtime.sendMessage({type:`PARK_TAB`,url:window.location.href,title:document.title,context:e}).catch(()=>{}),await S(`later`,{context:e}),x();try{let e=await chrome.runtime.sendMessage({type:`GET_CURRENT_TAB_ID`});e?.tabId&&await chrome.runtime.sendMessage({type:`CLOSE_TAB`,tabId:e.tabId})}catch{window.close()}},u.getElementById(`nevermind`).onclick=async()=>{await S(`nevermind`);try{let e=await chrome.runtime.sendMessage({type:`GET_CURRENT_TAB_ID`});e?.tabId&&await chrome.runtime.sendMessage({type:`CLOSE_TAB`,tabId:e.tabId})}catch{window.close()}};let w=u.getElementById(`dismiss`);w&&(w.onclick=async()=>{await S(`dismiss`),x()}),u.getElementById(`skip-domain`).onclick=async()=>{await chrome.runtime.sendMessage({type:`SKIP_DOMAIN`,domain:location.hostname}).catch(()=>{}),await S(`skip_domain`),x()},u.querySelectorAll(`[data-inherit-id]`).forEach(e=>{e.onclick=()=>C(e.querySelector(`.label`).textContent,e.getAttribute(`data-inherit-id`))}),u.querySelectorAll(`[data-preset]`).forEach(e=>{e.onclick=()=>C(e.getAttribute(`data-preset`))}),v.onkeydown=e=>{e.key===`Enter`&&u.getElementById(`continue`).click()}})();