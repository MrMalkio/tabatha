...
## [v6.7.78] - Elapsed clamp cleared to ship: cross-wire run recovery, stable pushed anchors - _2026-07-25_

> Final findings from the adversarial re-review of 6.7.77, which cleared the clamp to ship with these three landed. **N1 — the Sidecar's run-recovery formula had a broken input.** The Sidecar derives the current continuous run as `(now − _startedAt) − tags._elapsedMs`, but `buildFocusRows` published `_elapsedMs` only while a focus was *paused*, so on active rows the phone read banked as `0` and the run degenerated to the entire lifetime — K2 recurring across the wire, on the extension-authored rows that are 37 of the 38 in the working set. Measured: a 14 h-banked active item pushed a 14 h 05 m back-dated `_startedAt` with no `_elapsedMs`, the phone computed a 14.08 h "run", the ceiling fired, and pausing from the phone would have frozen 12 h — destroying 2.08 h. `_elapsedMs` is now published on every row, making `_startedAt` and `_elapsedMs` mutually consistent *by construction*; arbitration is unaffected (`focusRowStartedAtMs` never reads it) and no extension row ingests it back (`reconcileKnownFocusRow` only applies tag keys from Sidecar-sourced rows). **N2 — `clampStoredElapsed` was not the pure function its own docstring promised.** It defaulted `now` to `Date.now()`, so `life = now − anchor` grew every cycle and a legacy corrupt row published an anchor that *receded* about five minutes per five minutes (22:00:00, then 22:55:01, then 22:49:58) — any reader computing `now − _startedAt` saw elapsed climb at twice real time, and a paused corrupt row's published `_elapsedMs` crept upward each cycle until the clamp silently stopped firing. The `Date.now()` default is gone; callers must pass the item's own frozen instant (`lastResumedAt` while active, `pausedAt`/`endedAt` once paused). **N3** — the full-upsert path still re-slid a paused focus's `focus_timer_ends_at` on any identity change (the 6.7.77 guard only covered the 60 s heartbeat), so a paused deadline is now anchored to `pausedAt`; `adoptRemoteActive` no longer rewrites a non-parseable remote `_startedAt` into a fresh `now` anchor, degrading to a stable passthrough instead; and the Sidecar's `updateFocus` — the one writer that could move `_startedAt` without its banked partner — now writes a matching `_elapsedMs`. **On S4:** with N1 landed it is closed for both pause paths on both surfaces. The residual is the two *edit* paths that move an anchor independently of its banked value — the extension's `setFocusStartTime`, now correctly honoured by the `min(createdAt, startedAt)` life anchor, and the Sidecar's `updateFocus`, addressed by N3. Clamping to observed device liveness (the audit's own §S4 recommendation) remains the Plan 046 follow-up. 8 new tests (833 total), each verified red against 6.7.77 first. [tour: none]

## [v6.7.77] - Koda BLOCK remediation: two reproduced elapsed-clamp data-loss defects - _2026-07-25_

> Adversarial review of 6.7.74 returned **BLOCK** on the elapsed clamp, having reproduced two ways it silently destroyed real work — the exact failure the review gate exists to catch. **K1 — backdated work was deleted.** `setFocusStartTime` ("I was working before I created this focus") is a shipped button, and `validateStartTime` deliberately does *not* bound the chosen start by `createdAt` because being earlier IS the feature. The clamp measured life from `createdAt || startedAt` — the **later** anchor — so it was stricter than the edit it was validating: a focus created 14:00, backdated to 09:00 with 5 h credited, returned **5.0 minutes** at 14:05 and the next pause banked that, losing 295 minutes from disk. Now measured from `min(createdAt, startedAt)`, agreeing with the `wallClockMax` that the manual time-edit path already used, plus a `life > 0` guard so an item whose anchors are both null (where `buildFocusRows` stamps `created_at = now`) can't have its elapsed zeroed. **K2 — a lifetime was fed to a run-shaped clamp.** `adoptRemoteActive` set `lastResumedAt` to the remote's `tags._startedAt`, which is *back-dated by the whole accumulated elapsed* (the 6.7.71/6.7.73 contract), so `now − lastResumedAt` was a lifetime, not a run: a Sidecar intent worked 14 h across a week, adopted and paused five minutes later, was clamped to 12 h — destroying 2 h 05 m and making the browser read 12 h while the phone read 14 h, the very disagreement S9 exists to kill. Fixed at the data model rather than by offsetting the ceiling: adoption now banks the remote's accumulated time as `elapsedMs` and starts a genuine run at `now`, so the ceiling only ever sees real continuous run time. Push-identical, so the non-ping-pong invariant is untouched — `buildFocusRows` back-dates to byte-for-byte the anchor that was adopted, and a test asserts it. **P1 — the "never silent" promise was false.** `sanitizeFocusItem` coerced the `tags._elapsedClamp` audit stamp to `null` on the very next `getFocusEngine()` read, before it could ever be pushed; exempted by name, proven by a real storage round-trip rather than a unit test on the stamp builder. Also: `setSessionFromCompanion` was a third door for the S5 break-end regression (it rebuilt the session with no `breakEndedAt` and erased one a local break-end had just stamped); a paused focus's `focus_timer_ends_at` slid forward 60 s per heartbeat and rendered as a timer counting *up*; `buildFocusRows` published a raw `_elapsedMs` alongside a clamped `focus_elapsed_ms`; a stale `clocked_out` row is now exempt from the freshness horizon (it can only end a shift, never fabricate one — filtering it recreated the ghost-stint class); and a structurally-clamped sub-intent now credits its parent only what the child actually banked. 24 new tests (803 → 827), with K1, K2, P1 and P3 each verified red against 6.7.75 first. [tour: none]
...
## [v6.7.75] - Close the S5 adoption hole: clock adoption reconstructs breakEndedAt - _2026-07-24_

> Self-review of 6.7.74 found the S5 break-end regression sneaking back in through a second door. `clockService.applyRemoteClockState` rebuilds a local `clockSession` from a remote status row, and the remote row carries no `breakEndedAt` of its own — so an adopted `clocked_in` session derived its event time as plain `clockedInAt`, the exact backwards jump 6.7.74 fixed on the break path. It also broke stated invariant 1 (non-ping-pong, audit §1.2): adoption must carry the remote's own timestamp so the adopter's next push reproduces the identical value it read, otherwise the remote row stays permanently strictly-newer and re-signals adoption every cycle. Fixed by reconstructing `breakEndedAt` when the remote's `last_clock_event_at` is later than its `clocked_in_at` — which can only mean a break-end, since a break-start would have left the row `on_break`. 3 regression tests covering post-break, plain-clock-in (no phantom break invented) and on-break adoption. [tour: none]

## [v6.7.74] - Sync integrity cluster: clamp elapsed, monotonic clock events, freshness cutoff, live published elapsed - _2026-07-24_

> Four independent data-integrity defects from the 2026-07-24 cross-surface sync forensics (`docs/audits/2026-07-24-sync-forensics.md`), fixed together because they all corrupt the same two axes. **S4 (extension half only — see note)** — elapsed accrued raw wall-clock time with no ceiling, so a focus left `active` while the browser was closed billed the whole absence to one intent. New shared `src/utils/elapsedClamp.js` applies a structural clamp (elapsed ≤ the focus's own wall-clock life) plus a 12 h continuous-run ceiling chosen from the real distribution of all 310 rows carrying `tags._elapsedMs` — 8.23 h is the longest credible session, 20.69 h and 37.24 h are the only rows above 12 h and neither is believable as one unbroken run. Clamps are never silent: the rejected value is preserved in `tags._elapsedClamp` and warn-logged. *Note: the measured 37 h 14 m incident was written by the **Sidecar's** `pauseOtherActives`, which this commit does not touch — S4 is not closed by this entry alone; the Sidecar half ships separately as Sidecar 0.13.12.* **S5** — `last_clock_event_at` was `breakStartedAt || clockedInAt`, and since `clock.js` nulls `breakStartedAt` on break-end, ending a break made the published timestamp jump *backwards* to the shift start, letting a sibling's stale `on_break` row win arbitration and drag the just-resumed install back onto break. New `breakEndedAt` field plus a single `lastClockEventAt()` max-of-all-events helper shared by `awarenessService` and `liveIngestArbitration` so the two can't diverge again. **S7** — the clock ingest had no freshness cutoff at all (`pullClockCandidates` never even selected `last_heartbeat_at`), so a device dead for months while `clocked_in` stayed a candidate forever and beat a fresh install automatically; added a 90-minute staleness horizon, deliberately looser than the 5 min awareness threshold because an MV3 service-worker eviction silences a browser that is genuinely still in use. **S9** — `browser_profile_status.focus_elapsed_ms` published the stored-only elapsed, so it froze for the duration of every run while the adjacent `focus_timer_ends_at` already added the live portion; both now derive from one clamped `liveElapsed`. 62 new/updated unit tests, including a replay of the S4 incident with its real timestamps and a guard asserting the 12 h ceiling still clears every legitimate session observed in prod. [tour: none]

## [v6.7.74] - Pairing codes are no longer burned by a backend hiccup + real brute-force limiting on redeem - _2026-07-25_

> Two fixes in one change, because fixing the first activates the second. **(1) The bug.** "The pairing code expires instantly" was never expiry: `pair-watch` redeem set `consumed_at` BEFORE minting the session, so any failure in the mint path (the project's ES256 JWT-signing-key migration produced a window of 403 `bad_jwt`) spent the code permanently, and the retry matched no unconsumed row and reported "invalid or expired". Codes were BURNED, not expired. Replaced with a two-phase claim (migration 061): `claim_pairing_code` takes a short atomic lease (`UPDATE ... RETURNING`, so single-use stays race-proof), the session is minted, and only then does `consume_pairing_code` spend it; every failure path calls `release_pairing_code`, and a crash mid-flight resolves itself when the 30s lease expires. A backend problem can no longer cost a user their code. **(2) The vulnerability it exposes** (Koda F-EDGE-1, High). Redeem is unauthenticated by design, the 6-digit code IS the credential, and success returns `access_token` + `refresh_token` — full account takeover. The documented `attempts >= 5` lock was dead code: lookup is by SHA-256 hash, so a wrong guess matches no row, falls into `!row`, and increments nothing — 10^6 codes over a 300s window is ~3,400 req/s to exhaust, a commodity attack. Redeem now counts failures whether or not a row matched, in a new `tabatha.pair_redeem_attempts` ledger keyed by a SHA-256 fingerprint of the caller IP (never the IP itself, swept at 24h): 10 failures / 15 min per source, plus a 500 / 5 min global circuit breaker for the distributed variant, a per-code cap of 10 claims, `429` with `Retry-After`, and a gate that FAILS CLOSED — if the limiter can't be evaluated, redeem is refused rather than waved through. Every 500 branch now logs with diagnosable context and failed redeems are logged at all (previously an attack in progress was completely invisible). Also restores the Chrome Web Store extension id to the function's CORS allow-list — it exists in the deployed function but was missing from source, so a clean redeploy would have broken pairing for every store install. Client side, the extension's mint path now separates 429 and 5xx from "bad code" instead of collapsing them. [tour: none]
...
## [v6.7.69] - Self-heal InPop "[object Object]" data corruption (legacy-corrupted label/funnelStage/context) - _2026-07-23_

> Fixes the live-observed bug where the InPop gatekeeper rendered `[object Object]` for the intent input value, every paused-focus row's label, and every row's funnel-stage badge. Root cause: `gatekeeper.js`'s `escapeHtml()` faithfully renders whatever it's given (`String(anObject)` is literally `"[object Object]"`) — the DATA reaching it was object-shaped, not the render code. A full sweep of every current shipping writer of `label`/`funnelStage`/tab `context` (focusService.js, tabService.js, dataRehydrate.js, focusIngestService.js/liveIngestArbitration.js, every UI caller of `updateFocus`/`renameFocus`/`SET_TAB_CONTEXT`) found none that produce an object today — this is legacy/historical local-storage corruption that survives indefinitely because `reconcileKnownFocusRow` returns non-Sidecar-sourced items untouched and `dataRehydrate`'s newest-wins merge only overwrites when the cloud ref time is newer; neither path ever coerced an existing bad value back to a string, and `handleTabCreated`'s parent→child `context` inheritance silently propagates one corrupted ancestor tab's value to every tab opened from it. New pure module `src/utils/focusDataSanitize.js` (`coerceStringField`/`sanitizeFocusItem`/`sanitizeFocusEngine`/`sanitizeTabContext`/`sanitizeTabsMap`) coerces any object-shaped field to its best inner string (`{label|text|value|name}`) or a safe default (`label`→'Untitled focus', `funnelStage`→'unsorted', `context`→null), wired into `focusService.js` + `storageService.js`'s `getFocusEngine()` and `storageService.js`'s `getTabData()` so every install self-heals on its very next engine/tabs read — no reinstall, no data loss — plus defensive coercion at the two inbound-cloud-row mappers (`dataRehydrate.js serverFocusToLocal`, `liveIngestArbitration.js reconcileKnownFocusRow`) so re-corruption can't sneak back in. 16 new unit tests (`test/focusDataSanitize.test.js`) including a red→green repro against the real `focusService.getFocusEngine()` proving the self-heal persists the repair to storage.
...
- **Continuous focus ingest** — a 60s alarm (`focus-live-ingest`) plus a trigger right after every push cycle pull `focus_items` for the signed-in profile: a `created_at`-watermarked query for brand-new rows, plus a small `focus_state`+`tags`-only "light sweep" of every non-completed row to catch state changes (there's no `updated_at` column). Unknown rows are created locally via the same mapper the one-time rehydrate uses; known rows reconcile per a source-aware winner matrix — cloud wins for Sidecar-authored rows (`tags._src === 'sidecar'`), local wins for extension-authored rows.
- **Account-wide focus arbitration** — the account's single "current" focus is whichever active row (any source) has the latest `tags._startedAt`. When a newer remote active row appears, the extension pauses its local current (normal pause path — events still emit) and adopts the newer one through a new suppressed activation path (`adoptRemoteActive`) that never re-emits a `start` event and never stamps a fresh timestamp, so the next push is a no-op and no ping-pong loop forms.
- **Continuous clock ingest** — the same ingest cycle also pulls `browser_profile_status` rows (excluding self) and arbitrates the account's shift state by the latest `last_clock_event_at`, adopting a newer remote clock-in/break/clock-out into the local `clockSession` mirror (`clockService.applyRemoteClockState`) without touching `clockHistory` or re-pushing — the origin install's own history push remains the single source of truth for `clock_sessions`.
- The extension now also mirrors `tags._startedAt`/`tags._elapsedMs` on every focus push (`syncService.buildFocusRows`), matching the Sidecar's own convention, so extension-authored rows are comparable on equal terms during arbitration from any surface.
- `src/utils/liveIngestArbitration.js` — the pure comparator/reconcile-matrix logic, unit tested (30 tests, `test/liveIngestArbitration.test.js`) independent of chrome/supabase.
...
- **Tabby Sidecar gets a full detail page.** New `site/show/components-sidecar.html` (12 components, rendered from the app's real tokens — `sidecar/src/lib/theme.ts`, accent `#00E0D6`): sign-in, active focus & queue, voice capture and proactive check-ins, Tasks & Asana sync, Context View (including the phone-away red overlay), Phone Focus Mode, PWA install & self-healing, Web Push, in-app feedback, Notes/simple capture, watch pairing, and Cloud Sync. The `/show` family tile now links to this page instead of straight out to the live app; "Open Tabby Sidecar" remains as its own strip. Sidecar reflected at v0.7.2.
- **Tabby Watch joins the showcase family.** New `site/show/components-watch.html` (8 components, rendered from `tabatha-watch`'s Wear OS Compose theme — green/amber/red focus-ring states): the focus ring, action chips, checkpoint quick-add, shift glance, Tile, complication, password-free 6-digit pairing, and the architecture/fixture-mode card. Framed honestly throughout as a **sideload beta** — no store listing, built and sideloaded from `MrMalkio/tabatha-watch` (v0.1.2). A new family tile and a non-`.ready` get-strip point here; the family section is now "4 in the family".
- **Roadmap.** Replaced the Tabby Watch Todo card with a Resolved card (v0.1.2, sideload beta) and added a new Todo card for the store release that remains outstanding.
- **Search index & site build.** `scripts/build-search-index.mjs` now indexes both new pages (140 component cards total, up from 120); ran `npm run site:build` to regenerate `search-index.json` and `privacy.html`.
...
## [v6.7.19] - Teaser homepage, waitlist, showcase moves to /show - _2026-07-16_

### Added

- **Teaser homepage** (`site/index.html`) — the site root is now a held moment rather than a component gallery. It asks the Gatekeeper's own question ("What are you down for?") and answers it with a live instrument counting the visitor's real elapsed time on the page, captioned "Unaccounted for." One idea, one input, no feature dump. Tokens are lifted verbatim from the showcase's `:root` (2/4/8 radius, `#050505`, `#00F0FF`, the Inter / JetBrains Mono stacks) so the teaser and the product read as one system. Self-contained: no CDN, no external font, no analytics, no beacon.
- **Waitlist** (`functions/api/waitlist.js` + `supabase/migrations/028_waitlist.sql`) — the page's only CTA and only real action. Email posts to a Pages Function holding the service-role key in an env binding; **no Supabase key is ever shipped to the browser.** Server-side validation, per-isolate rate limiting, and duplicates treated as success via `ON CONFLICT DO NOTHING` so the endpoint can never be used to test whether an address is on the list. Returns 501 when unconfigured and the page says so plainly rather than faking a signup.
- **Privacy page** (`site/privacy.html`, generated) — `/privacy` is now a real page, rendered from `PRIVACY.md` by `scripts/build-privacy.mjs` so the published policy cannot drift from the repo's. Linked from the teaser footer and from the signup fine print, because the site now collects email addresses.
- **Waitlist section in `PRIVACY.md`** — covers what the address is used for (one email, once), that it is unlinked to any browsing data, that the list is not publicly readable, and how to be removed. The policy's scope line now names the website as well as the extension.

### Changed

- **`showcase/` is now `site/show/`, and `site/` is the deploy root.** Done as pure renames, so every one of the 24 files retains its history. No page content changed: every internal link and both data fetches were already relative, so `/show/` works exactly as `/` did. Deploy is now `wrangler pages deploy site`.
- **`_headers` moved to the deploy root** (`site/_headers`) and its data/asset rules repathed under `/show/`. Pages only reads `_headers` at the root of the deployed directory, so left in place it would have silently stopped applying: the CSP, the frame rules and the cache policy would all have quietly vanished.
- **`scripts/serve-showcase.mjs` now serves the whole deploy root**, resolving directory requests and clean URLs the way Pages does, so `/`, `/show/` and `/privacy` behave locally exactly as deployed. `/api/waitlist` is stubbed at 501 alongside `/api/feedback`.
- **`scripts/build-search-index.mjs` and `scripts/capture-screenshots.mjs`** read from `site/show/`. The five pinned Chrome Web Store screenshots are untouched: same filenames, same `store-assets/screenshots/` destination, same enforced 1280x800.

### Security

- **`tabatha.waitlist` is deny-by-default**: RLS enabled and forced, with **zero policies**. A waitlist signup is anonymous by definition, so there is no identity to scope a policy to and none is written; `service_role` bypasses RLS and is the single intended path in. The inherited `anon` / `authenticated` grants from migration 006's `ALTER DEFAULT PRIVILEGES` are explicitly revoked, so both locks must fail before an address could leak.
...
- **Site-wide search** — one box in the header of every real page (hub, all 6 category pages, roadmap), querying a prebuilt static index of **147 records**: all 90 component cards, 9 section descriptions, 8 surfaces, 6 pages, and every roadmap item. Fully keyboard-driven: `/` focuses from anywhere, arrows move, Enter opens, Escape closes. `npm run build:search-index` generates `showcase/search-index.json` by **parsing the real page markup** (`scripts/build-search-index.mjs`), so a card renamed in HTML cannot drift out of sync with what search returns; `npm run check:search-index` fails CI if it is stale.
- **Section purpose copy** — every section header and category page now states what the group of components *is* and what job it does for the user, derived from the shipped source and the four pillars in `Tabatha_Concept.md`.
- **Per-card Report bug / Request feature** — every one of the 90 cards carries both actions, bound to that exact component (id + name + page prefilled into the form). Injected at runtime from the card's own `id` and caption, so no per-card markup was needed. The controls are absolutely positioned and transparent until hover/focus, which keeps `.libcard` bounding boxes **byte-identical for the card captures**.
- **Feedback modal + `POST /api/feedback`** — a Cloudflare Pages Function (`functions/api/feedback.js`) files submissions as Asana tasks, reading its token from the `ASANA_TOKEN` env binding (never hardcoded, never logged). With no token configured it returns a clear **501** and the client falls back to opening a **prefilled GitHub issue**, so the feature works before the backend is wired. Also the fallback on `file://` and on any upstream error: the reporter's words are never lost.
- **Header "Request a feature"** — the same modal from every page, with no component prefilled.
- **Roadmap page** (`showcase/roadmap.html`) — a kanban board whose columns are the product's **real funnel stages** (`FUNNEL_STAGES` in `src/hooks/useFocusEngine.js`: unsorted, todo, focus, addressing, roadblocked, resolved) with their shipped labels, emoji and colors. Fed by a **curated** `showcase/roadmap.json` (34 entries) — deliberately not wired to the internal board, so no internal task or client names can leak.
- **Cloudflare Pages deployment** — `showcase/_headers` (CSP, no external origins, `frame-src 'self'` for the embedded surfaces), plus deploy, env-var and roadmap-update docs in `showcase/README.md`. `npm run site:serve` serves `showcase/` over http locally (search and the roadmap need a real origin) and stubs `/api/feedback` at 501 to exercise the fallback.

### Fixed

- **Showcase search: HTML entities rendered literally** — card titles containing `&mdash;` were indexed raw, so results read `Gatekeeper &mdash; agent` and a search for `gatekeeper agent` could not match across the entity. The index builder now decodes named, decimal and hex entities.
