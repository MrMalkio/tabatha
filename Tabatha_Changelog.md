...
> Four independent data-integrity defects from the 2026-07-24 cross-surface sync forensics (`docs/audits/2026-07-24-sync-forensics.md`), fixed together because they all corrupt the same two axes. **S4** — elapsed accrued raw wall-clock time with no ceiling, so a focus left `active` while the browser was closed billed the whole absence to one intent (measured: **37 h 14 m** on `f_1784676002315_yo37y`, matching its causing event to within 80 ms). New shared `src/utils/elapsedClamp.js` applies a structural clamp (elapsed ≤ the focus's own wall-clock life) plus a 12 h continuous-run ceiling chosen from the real distribution of all 310 rows carrying `tags._elapsedMs` — 8.23 h is the longest credible session, 20.69 h and 37.24 h are the only rows above 12 h and neither is believable as one unbroken run. Clamps are never silent: the rejected value is preserved in `tags._elapsedClamp` and warn-logged. **S5** — `last_clock_event_at` was `breakStartedAt || clockedInAt`, and since `clock.js` nulls `breakStartedAt` on break-end, ending a break made the published timestamp jump *backwards* to the shift start, letting a sibling's stale `on_break` row win arbitration and drag the just-resumed install back onto break. New `breakEndedAt` field plus a single `lastClockEventAt()` max-of-all-events helper shared by `awarenessService` and `liveIngestArbitration` so the two can't diverge again. **S7** — the clock ingest had no freshness cutoff at all (`pullClockCandidates` never even selected `last_heartbeat_at`), so a device dead for months while `clocked_in` stayed a candidate forever and beat a fresh install automatically; added a 90-minute staleness horizon, deliberately looser than the 5 min awareness threshold because an MV3 service-worker eviction silences a browser that is genuinely still in use. **S9** — `browser_profile_status.focus_elapsed_ms` published the stored-only elapsed, so it froze for the duration of every run while the adjacent `focus_timer_ends_at` already added the live portion; both now derive from one clamped `liveElapsed`. 62 new/updated unit tests, including a replay of the S4 incident with its real timestamps and a guard asserting the 12 h ceiling still clears every legitimate session observed in prod. [tour: none]
