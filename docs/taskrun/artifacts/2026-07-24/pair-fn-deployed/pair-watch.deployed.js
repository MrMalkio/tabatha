...
Deno.serve(async (req)=>{
  ...
  if (body.action === "redeem") {
    ...
    if (!/^\d{6}$/.test(code)) return json({
      error: "invalid code"
    }, 400, cors);
    const hash = await sha256Hex(code);
    const { data: row } = await admin.from("watch_pairing_codes").select("id, profile_id, expires_at, consumed_at, attempts, device_label").eq("code_hash", hash).is("consumed_at", null).maybeSingle();
    ...
    if (linkErr || !link?.properties?.hashed_token) return json({
      error: "pairing failed"
    }, 500, cors);
    ...
    if (otpErr || !session?.session) return json({
      error: "pairing failed"
    }, 500, cors);
    return json({
