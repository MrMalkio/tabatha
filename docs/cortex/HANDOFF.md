...
## ⚡⚡⚡ Third round (blank home again / intent not saving / companion / DB) — 2026-07-10 midday
Diagnosed on your LIVE machine (Chrome CDP + a read-only copy of the extension's LevelDB — no secrets printed).

1. **Blank home page: root cause + fix (`cf09b04`).** Voice v0's new `VoiceNoteButton` used `useRef` without importing it → ReferenceError blanked the whole home render. Settings/sidebar have separate bundles, so they survived. Fixed and re-verified in a real browser (11/11). **This was almost certainly also why "creating an Intent did nothing" and buttons looked dead** — on a crashed home page nothing is interactive; the InPop/InBar intent paths themselves test clean.
2. **Sync stale — real root cause found in your storage.** Your Supabase auth token (`sb-…-auth-token`) is **wiped** (`hasUser:false`, no refresh token) — the session got force-removed, so every sync bails `no_auth_session` / `auth.getSession timed out`. Your last success was 05:15 today. **Fix: sign back in** (Settings → Sync & Account → sign out, then in). Contributing and now fixed by me: (a) `chrome.storage.local` hit **quota-exceeded** on 07-06 — added `unlimitedStorage` to the manifest (`f127e09`); (b) the header Sync-Now button silently swallowed failures — it now shows the actual diagnostic (`f127e09`).
3. **DB is behind and I'm BLOCKED on credentials.** Your sync log shows `focus_items.priority` missing → **migration 021 was never applied** (nor 020, and the new cortex 022/023). This causes partial sync failures even after you re-auth. I could NOT push: the Supabase MCP connector is scoped to a *different* project ("Caspera Fadena", not Flux `mtdgoahskcibjbhfvofx`), the `sbp_` access token in `.env.cortex.local` returns 401, and there's no DB password on disk. **You (or a valid token) must run:** `$env:SUPABASE_DB_PASSWORD='<Flux pw>'; npx supabase db push --linked` — applies 020/021/022/023. Or paste me a working Flux access token and I'll push.
4. **Companion.** You were running the OLD debug build (crash-hardening never packaged). I built the **release** exe (`tabatha-desktop\src-tauri\target\release\tabatha-desktop.exe`), launched it (WS bridge reconnected cleanly), and repointed your auto-start registry key to it. The "desk panel" is a dev-only debug UI (loads from a Vite dev server that isn't running in release) — not part of normal use; the tray app + capture + bridge are what matter and they're live.
5. **ElevenLabs.** Authed in your browser and minted a **scoped** key ("Tabatha Cortex", Text-to-Speech + Speech-to-Text only, everything else No Access), stored to the gitignored `.env.cortex.local` `ELEVENLABS_API_KEY` slot (never printed; clipboard cleared). API-KEYS.md K10 marked ✅.
6. **New feature scoped + v1 built:** agent-vs-human attribution (your ask) — see `C11a-agent-interaction-surfaces.md` + the agentSession service/UI.

## ⚡⚡ Second round (blank home / stale sync / companion died) — resolved 2026-07-10 morning
1. **Blank home page: real bug, fixed** (`cf09b04`). Voice v0's `VoiceNoteButton` used `useRef` without importing it — ReferenceError killed the home render (settings/sidebar have their own bundles). Build+tests can't catch a missing hook import; re-verified in a real browser after the fix: home renders, focus start/pause/resume and Set Focus all pass (harness `focus.js`/`setfocus.js`; `run.js`'s T4 "fails" are its own known-bad first-pass assertions).
2. **Companion closed itself / desk panel dead: pre-existing.** The deployed companion is the OLD build — the FIX-06 crash-hardening commits were never packaged. Verified the old binary safely ignores the new `CAPTURE_CONFIG` message (logs a warning, no crash), so Cortex didn't kill it. Relaunch it for now; the durable fix is deploying `feat/cortex-capture` (which sits on top of the FIX bundle).
3. **Sync stale: nothing tonight touched sync** (cortexLedger deliberately isn't in DURABLE_SYNC_KEYS, so capture writes can't storm/starve the debounce — verified). Most likely an expired auth session. Diagnose: chrome://extensions → Tabatha → "service worker" → look for sync/auth errors when you click Sync now; if it's auth, Settings → "⚠ Force reset auth" then sign back in. If the SW console shows something else, paste it to me.
