...
6. **Migration push (020–023) — status: in flight / unresolved as of this writing.** Fable was blocked (Supabase MCP scoped to the wrong project, `.env.cortex.local` `sbp_` token returns 401, no DB password on disk). No evidence in git status/docs of a completed push since HANDOFF.md was written. Run: `$env:SUPABASE_DB_PASSWORD='<Flux pw>'; npx supabase db push --linked`, or hand Fable a working Flux access token. Gates: sync fully succeeding even after re-auth (`focus_items.priority` missing today), migrations 022 (ledger) + 023 (org_capture_policy) going live.
...
- Migration gap (020–023 unapplied to remote Flux) causes partial sync failures even after re-auth — see waiting-on #6.
