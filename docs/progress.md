...
## 2026-07-23 (overnight TaskRun) — CeeCee/Opus: 16/16 queue addressed; extension source FORK is the headline
- **Fork discovery (headline):** `staging` src is 6.7.47 with only the compiled 6.7.56 CRX binary; `integrate/6.7.50` holds the real 6.7.48–6.7.56 SOURCE (XSS sweep, RLS guard, device pairing codes, logo). Forked at `59d326b`, neither a superset. Queue TR-18 had it backwards (integrate is canonical, NOT superseded). All ext fixes based on integrate, v6.7.57+, local only. See `docs/taskrun/2026-07-22-morning-{questions,report}.md` + `2026-07-22-git-reconciliation.md`.
- **Shipped (built+verified+committed, NOT deployed):** TR-02 Companion 0.3.9 (`f25d1c0`); TR-05 Watch 0.2.1 runCatching crash-guard, 27 tests (`5f8977f`); TR-17 Sidecar 0.13.6 version-from-app.json (`c3bc8de`); TR-14b Sidecar 0.13.7 header feedback button, healthy expo bundle (`9ea5f96`); TR-16 site:deploy --branch=main (`5385346`+`8c92cde`); TR-15 feedback-review doc (`4e9b0b8`); extension S-batch TR-04/06-12 6.7.57-6.7.64 (`2495db3..9670fd0`); extension M-items TR-03/13/14a 6.7.65-6.7.67 (`be45b99`,`e5c0974`,`31d9011`), self-reviewed PASS (Koda review agent died → authorized self-review).
- **Prepped for morning approval:** TR-01 Regina device-dedup (18 unreferenced dupes→19 kept, `scripts/regina-device-dedup-2026-07-23.sql`, `8ed8c6e`); TR-18 reconciliation note (`cc3f95c`).
- **Fork-blocked:** TR-19 (logo/docs drift — entangled with Q1/Q6).
...
## 2026-09-23 — Koda: Plan 050 release results and remaining gates

Ready source was promoted through PR 39 into staging and PR 40 into main at 333d99a. Extension 6.7.83 is live on the staff and enterprise channels; website Pages deployment ad355e83-9f8a-4a0a-9a1e-73dd1617b750 is successful. Live artifact hashes and inspected site bytes match the committed release. Cloudflare's commit_dirty metadata is disclosed in the ledger; no unexpected deployed content was found. CWS accepted upload/publish for the existing trusted-testers item and independently reports crxVersion 6.7.83; tester install availability is not proven by that API. Sidecar 0.13.14 was already live and remains byte-identical to the recovered source bundle.

Final combined extension/guard suite: 846/846 pass; Sidecar: 166/166 pass. Production builds, version/changelog checks, docs registry and isolated real Chrome smoke passed. The Cloudflare release checks motivated preserving and testing the enterprise anti-rollback guard; it now fails closed when live version verification fails. This does not replace signed-in cross-device regression.

All five original worktrees are clean. Original dirty material is preserved in local backups and pushed Koda/recovery-* refs; no historical worktree/branch was removed. Main working checkout contains the released runtime source, and its fixed dist path was updated atomically from the tested build. Its prior dist remains backed up. User-profile Chrome interaction stopped when user activity was detected; installed version/reload remains unverified. Vendor instruction copies now match the canonical session record.

Plan 050 remains partial (6/7): inventory/recovery/validation/final ledger complete, ready publication complete, authenticated rollout/backend validation gated. Next: check the installed extension is 6.7.83 and smoke-test signed-in Sidecar sync; restore valid Supabase management access; remediate pairing fail-open behavior and Asana resource authorization before considering those recovered changes for deployment. Do not replay historical migrations or reactivate rejected org-hours UI. Detailed hashes, recovery refs, rollback and next gates: `docs/reconciliation/2026-09-23-reconciliation.md`.

## 2026-09-24 — Claude (Fable 5.1): "intents don't update" root-caused to a full disk poisoning storage; self-heal + fleet diagnostic (6.7.84)

**Goal.** Malkio: "Tabatha seems to be broken … no intents are updating when I resolve or add them." Also asked to look at the DB.

**Root cause (proven).** `%LOCALAPPDATA%\Google\Chrome\User Data\Default\Local Extension Settings\jbdkacccpknbiphigeabcdojemnhacjj\LOG` line 6521: `2026/09/23-16:32:08 Compaction error: IO error: …\218365.ldb: FILE_ERROR_NO_SPACE`. C: had 0.2 GB free of 922 GB (still full at diagnosis time). LevelDB latches that background error, so every later `chrome.storage.local` write rejected for 25+ hours while reads worked: the service worker kept heartbeating and pushing its frozen snapshot every 2 min (cloud showed all 364 focus rows re-stamped with unchanged content), and every add/resolve/pause threw on save. Store WAL byte-identical across 3 h (read-to-EOF, not stat). A second extension's store logged NO_SPACE at 16:31:20 the same day and on 09-22 15:09 → system-wide.

**Ruled out along the way (with evidence).** LevelDB corruption; storage quota (6.7.56 has `unlimitedStorage` + the loud guard); capture-I/O mutex blocking focus writes (`serialized()` is local to captureService); a second writer on Malkio's profile (only `2b37076e`, this box, live in 48 h; `e9e80111` is another profile); harness Chromes (the 4 headless-shell processes are hyperframes preview servers, no extension inside); schema drift (migrations 001–060 aligned); the manifest-key "fork" as a CRX blocker (Chrome rewrites the key on install — OPERATIONS §2.2b was right, my earlier claim was wrong).

**Still open.** The fleet install never took a channel update after 6.7.56 (Default: installed at 6.7.56 on 07-21, only `6.7.56_0` on disk; Profile 2: updated 6.7.22 → 6.7.53 → 6.7.56 then stopped) although cached cloud policy references jbdka + update.xml (refreshed 09-23 15:58) and the 6.7.56→6.7.83 manifest diff is key-order only (no permission or version-floor change). Cause is not on disk; next test is Malkio's: `chrome://extensions` → Developer mode → Update, then `chrome://policy` / `chrome://extensions-internals`. Two dead cards remain (`hoknmocl…` unpacked→dist, frozen 07-22; `dphebjbo…` DISABLE_RELOAD, frozen 06-30).

**Shipped (6.7.84, branch `fix/storage-write-self-heal`).**
- `storageService.setStorage`: disk-full-aware notification; after 3 consecutive failures spanning ≥60 s, probe `chrome.storage.sync` — success ⇒ `chrome.runtime.reload()` (30-min cooldown marker in sync), failure ⇒ disk still full, notify only. Tests: `test/storageWriteSelfHeal.test.js`.
- `scripts/check-fleet-install.mjs` + pure `scripts/lib/fleet-install.mjs` (`npm run fleet:check`): per-profile extension entries/ghosts, on-disk fleet version vs live channel, cached-policy presence, disk free, WAL growth (read-to-EOF), LevelDB LOG classification. Tests: `test/fleetInstallHelpers.test.js`. Run against this box it reproduces every finding above.
- OPERATIONS.md §2.2b: client-side requirements + stuck-install diagnosis + the disk-full incident.

**DB findings (Supabase, linked CLI).** Single live extension writer on profile `4abda377` (`2b37076e`, local_id `4b0af0f0`, machine `00741d2e` = OD). Resolve worked end-to-end on 09-22 ("TGC pricing"). 364 focus rows, 0 `[object Object]` in Malkio's rows (present in another profile's `intent_history`). Install re-identified 09-12 03:30 (row `e428bd2a` "OD - Dev extension" → `2b37076e`). Revoked companion `00741d2e` (07-21) still writes `desktop_activity` (last 09-23 20:31).

**Valeted** (parking lot): newest-wins merge uses `>=` with the server's `synced_at` echoed as local ref time and nothing ever stamps `updatedAt` on focus items (latent multi-writer clobber); revoked companion still writing; ~487 KB/25 s storage churn seen on 6.7.56; install re-identification 09-12; fleet update trigger dead after 6.7.56.

**Malkio's steps.** 1) Free disk space (LOCALAPPDATA: Chirp 35.9 GB, Docker 30 GB, Packages 22.7 GB, hermes 13.3 GB, Temp 4.1 GB incl. 10 stale puppeteer profiles). 2) `chrome://extensions` → ↻ Tabatha (jbdka…). 3) Remove the two dead cards. 4) Developer mode → Update; confirm a `6.7.83_0`/`6.7.84_0` dir appears; else read `chrome://policy` and `chrome://extensions-internals`. 5) Re-test add/resolve. 6) `npm run fleet:check`.

**Correction (same day, 6.7.85).** The org install (`jbdka…`) is force-installed and force-pinned by Workspace policy from a **custom URL** (`tabatha.pondocean.co/enterprise/update.xml`), configured 2026-07-17 in Google Admin → Devices → Chrome → Apps & extensions → Users & browsers, Duck & Shark OU — confirmed in the cached `Default/Policy/User Policy` (refreshed 09-24 15:10). It is not the Chrome Web Store item; `piopncjac…` is published to trusted testers but no org policy references it. A policy install has no ↻ button and can't be disabled, so the manual recovery is a full Chrome restart (⋮ → Exit), not "reload the extension". `hoknmocl…` is the original 06-30 team-deploy unpacked copy that the deployment session said to remove once the managed one took over; `dphebjbo…` is an older stray unpacked copy. Both are safe to remove. Channel serving re-verified: update.xml `application/xml` 200, 6.7.83 CRX `application/x-chrome-extension` 200. Why Chrome stopped updating at 6.7.56 (both profiles) is still unexplained.
