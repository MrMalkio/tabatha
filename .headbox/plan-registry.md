...
| 042      | conversational_tabatha       | 2026-07-20 | Conversational Tabatha (the AI pillar) — default assistant managing Tabatha via conversation not direction; realtime AI + rotating preset spoken prompts across all proactive comms (incl. personality interrupts/#182); in-app "calls" + 5-10min intake conversations routing Tabatha-operational vs Flux-personal content (REQUIRES active Flux account, v2); Dispatch/PTT mode (push-speak-release, lightweight transcription, no conversational feedback, extension + Sidecar); transcription-provider abstraction (Web Speech API is the real v1 default — brief assumed assembly.ai already live, it is not; assembly.ai is the first pluggable alt); Unit 0 fixes 2 reported voice bugs (transcription repeats leading words; check-in cuts off mid-speech + ignores existing checkpoint content); CeeCee's device-handoff micro-summary idea. docs/superpowers/specs/2026-07-20-plan-042-conversational-tabatha-design.md | vetted (Koda) — proceed, Unit 0 diagnosis verified exact against shipped code; minor revises (Dispatch needs a visible non-TTS confirmation strip, cold-start gap in Unit 5) |
| 043      | peer_view                   | 2026-07-20 | Peer View — Context View variant for a non-user viewer (assistant/partner/friend). Reuses pair-watch's code mint/redeem UX but issues a scoped anonymous session (new `peer_grants` table) instead of a real user session — per-element CV visibility, per-grant capabilities (add intent/task, update checkpoint, nudge), user-set expiration, multi-peer management, peer nudge button (rides #182 audio channel). Tabatha-to-Tabatha real-account peers + Olympus pro-gating deferred to v2. Migrations claim 051-052 (placeholder, re-verify at build time). docs/superpowers/specs/2026-07-20-plan-043-peer-view-design.md | vetted (Koda) — revise-with-exact-revision: trust model is sound (verified no auto-profile-creation trigger exists) but `peer_grants` needs a brute-force `attempts` column (dropped vs. the `watch_pairing_codes` pattern it claims to reuse), the RPC liveness WHERE clause must be spelled out literally not left as prose, and `enable_anonymous_sign_ins` is currently `false` in config.toml |
| 044      | scheduling_calendar_presets  | 2026-07-20 | Scheduling, Calendar & Preset Checkpoints — continues Plan 035 Phases 2-3 (Calendar UI + Google OAuth engine, poll-based sync) rather than re-deriving them; scheduled future intents (new `scheduled_at` on focus_items + activation cron + alert); push a scheduled intent into the calendar (native or Google); preset checkpoints (new `preset_checkpoints` table, user/AI-defined, linked to subtasks, expected-time hit/miss tracking via sweep cron). CeeCee's auto-carve-focus-blocks idea + AI-authored presets deferred to v2. Migrations claim 053-055 (placeholder); shares `integration_credentials.provider` CHECK-widening coordination point with Plan 045. docs/superpowers/specs/2026-07-20-plan-044-scheduling-calendar-preset-checkpoints-design.md | vetted (Koda) — revise-with-exact-revision: `preset_checkpoints` §4.1 CHECK constraint is inverted (rejects the two valid single-value cases, allows the invalid both-set case — exact fix given in the vet section); Google OAuth token-refresh flow needs its own sub-step (unlike the non-expiring Asana PAT it's modeled on) |
| 045      | growth_integrations_governance | 2026-07-20 | Growth & Integrations governance — (a) invite quotas (5/non-owner, all-time mint count, owners unlimited, self-contained v1 check + new `invite_redemptions` invite-graph table, v2 wires to Olympus Provisioning); (b) task-sync provider abstraction (widens Asana-hardcoded CHECKs, new `TaskProvider` client interface, Anasa as real 2nd provider pending an API/auth audit, Notion/ClickUp/Google Tasks/Monday as "coming soon" catalog rows); (c) feedback/bug-report upgrade (Title field, per-kind Asana custom-field mapping pending a live field-GID audit — brief's assumption that GIDs already exist in code was verified false, only a project GID secret exists today — plus user name+email via existing `profiles.display_name`, not just user ID). Migrations claim 056-057 (placeholder). docs/superpowers/specs/2026-07-20-plan-045-growth-integrations-governance-design.md | vetted (Koda) — revise-with-exact-revision: invite-quota count-then-insert has a TOCTOU race (advisory-lock fix given in the vet section); all factual claims (create_invite_token signature, feedback-to-asana payload shape, zero Asana field GIDs in code) verified accurate |
| —        | fix_wave_3                  | 2026-07-20 | Fix Wave 3 (not a numbered plan) — CV timer full h:mm:ss digits + user-configurable count up/down + precision setting; un-resolve intents (Sidecar ports the extension's existing confirm-gated reopen pattern, which Sidecar never had); edit description of non-active/queued intents (EditPanel today only ever renders for the single "current" focus); timeline day/week/month separators for piecemeal long-running intents (FocusTimeline's axis becomes boundary-aware, not just duration-fractional); phone-off false "phone away" (new heartbeat-write + away-vs-gone semantic split, replacing the 30-min stale-read guess) + device type/priority categorization (uses the `device_settings` JSONB migration 045 already left open for exactly this — no new migration). Zero new migrations required. docs/superpowers/specs/2026-07-20-fix-wave-3-spec.md | vetted (Koda) — proceed, all 5 items verified accurate line-for-line against shipped code, no revisions required |
...
| #216      | Peer View                             | **043**                      | draft         |
| #217      | Dispatch Mode (Push-to-Talk)          | **042**                      | draft         |
| #218      | Preset Checkpoints                    | **044**                      | draft         |
| #219      | Tabatha Calls & Intake Conversations  | **042** (v2)                 | draft         |
| #220      | Task-Sync Provider Abstraction        | **045**                      | draft         |
