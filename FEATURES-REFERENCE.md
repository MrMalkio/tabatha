...
| Checkpoint Progress Notes (#184) | **WORKING** — plan-registry correct; the `184-*.md` doc saying "Planned" is stale | `SAVE_CHECKPOINT_NOTE`/`GET_CHECKPOINT_STATUS` fully implemented and UI-wired (`focusService.js:1377-1482`) | Tabatha periodically nudges you to log a progress note; it can auto-post to a linked Asana task. |
| Persistent Focuses (#225) | **NOT FOUND** | No ongoing/"done for today"/dynamic-preset lifecycle in `src/`; plan-registry lists #225 as unassigned. Plans 025 and 031 delivered the Checkpoint Progress Notes half only — this row was previously conflated with #184 | Focuses that run indefinitely (rather than to a timer) and can be marked "done for today" aren't built yet. |
| Focus Auto-Resume & Queue (#226) | **WORKING** — plan-registry correct; doc stale | Two real mechanisms: auto-promote on complete (`focusService.js:481-498`) and alarm-recalculated resume (`RESUME_BACKBURNER`) | Finishing your current focus pulls up whichever other focus you paused most recently. |
